...
class OutputProjection(nn.Module):
    ...
    def __init__(self, n_codebooks: int, codebook_size: int, d_model: int) -> None:
        ...
        self.connections = nn.ModuleList([nn.Linear(codebook_size, d_model) for _ in range(n_codebooks-1)])
        self.projection = nn.ModuleList([nn.Linear(d_model, codebook_size) for _ in range(n_codebooks)])

    def forward(self, x):
        """
        x: (B, T, D) --> logits: (B, N, T, C)
        Stack N layers, one for each codebook, projecting transformer output into codebook indexes
        """
        logits = [proj(x) for proj in self.projection]
        for i in range(self.n_codebooks - 1):
            tmp = self.connections[i](logits[i])
            logits[i+1] = self.projection[i](x + tmp)
        logits = torch.stack(logits, dim=1) # logits.shape==(B, N, T, C)
        return logits
...
class Transformer(nn.Module):
    ...
    def __init__(self, config, n_codebooks, codebook_size, sample_rate, frame_dim):
        ...
        self.output_projection = OutputProjection(n_codebooks, codebook_size, self.d_model)
    ...
    def forward(self, x):
        ...
        x = self.positional_encoding(x) # (B, N, T) --> (B, T, D)
