...
class OutputProjectionRNN(nn.Module):
    def __init__(self, n_codebooks: int, codebook_size: int, d_model: int, hidden_size: int = 512) -> None:
        super().__init__()
        ...
        self.rnn = nn.LSTMCell(d_model, hidden_size)
        self.to_logits = nn.Linear(hidden_size, codebook_size)
        self.to_states = nn.Sequential(
    ...
    def forward(self, x):
        ...
        hx, cx = torch.split(self.to_states(x), self.d_model, dim=-1)
        for i in range(self.n_codebooks):
            hx, cx = self.rnn(x, (hx, cx))
            logits_i = self.to_logits(hx).reshape(B, S, -1)
